# Decision Neuroscience Lab | 决策神经科学实验室

**Bridging Neuroeconomics, Deep Learning, and Behavioral Science.**
**连接神经经济学、深度学习与行为科学。**

---

### Our Mission | 我们的使命

**[EN]** We are dedicated to advancing the frontiers of decision science by integrating computational modeling with large-scale behavioral data. Our lab develops next-generation research tools designed to make high-precision cognitive measurement more accessible and scalable.

**[CN]** 我们致力于通过将计算建模与大规模行为与多模态数据（眼动、鼠标轨迹、脑电、功能核磁共振）相结合，推动决策科学的前沿研究。实验室专注于开发下一代研究工具，旨在使高精度的认知测量技术变得更加易于获取且具备可扩展性。

---

*Advancing the science of choice and decision making through computational innovation.*
*通过计算创新推动决策科学的发展。*
